#!/usr/bin/env python3
"""Validate Diamond v1 excluded-scope policy contract."""
...
def _find_keyword_hits(path: Path, banned_tokens: list[str]) -> list[str]:
    ...
    for ln, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if match:
            errors.append(f"{path}:{ln}: out-of-profile scope token: {match.group(1)}")
    return errors
...
def validate_policy(
    ...
    for key in sorted(REQUIRED_SCOPE_KEYS):
        if scope.get(key) != "none":
            errors.append(f"scope.{key} must be 'none'")
    ...
    tooling_ref = enforcement.get("tooling_policy_ref")
    ...
    for key, ref in (
        ...
        ("enforcement.tooling_policy_ref", tooling_ref),
    ):
    ...
    tooling_text = _read_text(repo_root / str(tooling_ref), errors, "enforcement.tooling_policy_ref")
    ...
    for clause in required_sem_clauses:
        if clause not in sem_text:
            errors.append(f"semantic contract missing clause: {clause}")

    required_obj_clauses = (
        "Classical class inheritance is out-of-profile for V1.",
        "Trait/interface system is out-of-profile for V1.",
    )
    for clause in required_obj_clauses:
        if clause not in obj_text:
            errors.append(f"object contract missing clause: {clause}")

    required_tooling_clauses = (
    ...
    for clause in required_tooling_clauses:
        if clause not in tooling_text:
            errors.append(f"tooling policy missing clause: {clause}")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate Diamond excluded-scope policy.")
    ...
    if errors:
        print(f"scope_policy: FAIL errors={len(errors)}")
        for err in errors:
            print(f"error: {err}")
        return 1

    print("scope_policy: OK profile=v1 checks=meta+effects+inheritance+tooling_scope")
    return 0
